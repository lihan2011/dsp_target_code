...
static bool isHardwareLoop(const MachineInstr *MI) {
  return MI->getOpcode() == DSP::LOOP0_r ||
    MI->getOpcode() == DSP::LOOP0_i;
}
...
bool DSPHardwareLoops::findInductionRegister(MachineLoop *L,
  ...
  unsigned PredR = Cond[CSz - 1].getReg();
	if(DSPDEBUG)
		std::cout << "Cond Register: " << PredR << std::endl;
...
CountValue *DSPHardwareLoops::getLoopTripCount(MachineLoop *L,
  ...
  // Branches of form "if (!P) ..." cause DSPInstrInfo::AnalyzeBranch
  ...
  switch (CondOpc) {
    ...
    case DSP::CMPbEQri_V4:
    case DSP::CMPhEQri_V4: {
      if (IVBump != 1)
        return nullptr;
      ...
      if (InitV >= EndV)
        return nullptr;
      if (CondOpc == DSP::CMPbEQri_V4) {
        if (!isInt<8>(InitV) || !isInt<8>(EndV))
          return nullptr;
      } else {  // DSP::CMPhEQri_V4
...
CountValue *DSPHardwareLoops::computeCount(MachineLoop *Loop,
  ...
  if (Start->isReg()) {
    ...
    if (StartValInstr && StartValInstr->getOpcode() == DSP::TFRI)
      Start = &StartValInstr->getOperand(1);
  }
  if (End->isReg()) {
    const MachineInstr *EndValInstr = MRI->getVRegDef(End->getReg());
    if (EndValInstr && EndValInstr->getOpcode() == DSP::TFRI)
  ...
  // will be "reg - imm".  DSP's "subtract immediate" instruction
  // is actually "reg + -imm".
  ...
  const TargetRegisterClass *IntRC = &DSP::IntRegsRegClass;
  ...
  if (Start->isImm() && StartV == 0) {
    ...
    const MCInstrDesc &SubD = RegToReg ? TII->get(DSP::SUB_rr) :
                              (RegToImm ? TII->get(DSP::SUB_ri) :
                                          TII->get(DSP::ADD_ri));
  ...
  if (AdjV == 0) {
    ...
    const MCInstrDesc &AddD = TII->get(DSP::ADD_ri);
  ...
  if (IVBump == 1) {
    ...
    const MCInstrDesc &LsrD = TII->get(DSP::LSR_ri);
...
bool DSPHardwareLoops::convertToHardwareLoop(MachineLoop *L) {
  ...
  if (TripCount->isReg()) {
    ...
    unsigned CountReg = MRI->createVirtualRegister(&DSP::IntRegsRegClass);
    BuildMI(*Preheader, InsertPos, DL, TII->get(TargetOpcode::COPY), CountReg)
      .addReg(TripCount->getReg(), 0, TripCount->getSubReg());
    // Add the Loop instruction to the beginning of the loop.
    BuildMI(*Preheader, InsertPos, DL, TII->get(DSP::LOOP0_r))
      .addMBB(LoopStart)
      .addReg(CountReg);
  } else {
    ...
    if (!TII->isValidOffset(DSP::LOOP0_i, CountImm)) {
      unsigned CountReg = MRI->createVirtualRegister(&DSP::IntRegsRegClass);
      BuildMI(*Preheader, InsertPos, DL, TII->get(DSP::TFRI), CountReg)
        .addImm(CountImm);
      BuildMI(*Preheader, InsertPos, DL, TII->get(DSP::LOOP0_r))
        .addMBB(LoopStart).addReg(CountReg);
    } else
      BuildMI(*Preheader, InsertPos, DL, TII->get(DSP::LOOP0_i))
  ...
  BuildMI(*LastMBB, LastI, LastIDL,
          TII->get(DSP::ENDLOOP0)).addMBB(LoopStart);

  // The loop ends with either:
  //  - a conditional branch followed by an unconditional branch, or
  //  - a conditional branch to the loop start.
  if (LastI->getOpcode() == DSP::JMP_t ||
      LastI->getOpcode() == DSP::JMP_f) {
...
MachineInstr *DSPHardwareLoops::defWithImmediate(unsigned R) {
  ...
  switch (DOpc) {
    case DSP::TFRI:
    case DSP::TFRI64:
    case DSP::CONST32_Int_Real:
    case DSP::CONST64_Int_Real:
      return DI;
  }
  return nullptr;
}
...
bool DSPHardwareLoops::fixupInductionVariable(MachineLoop *L) {
  ...
  // DSPInstrInfo::AnalyzeBranch for negated branches inserts imm(0)
