	...
	public:
		...
		bool runOnMachineFunction(MachineFunction &mf) override {
			...
			for (MachineFunction::iterator MBBI = mf.begin(),MBBe = mf.end(); MBBI != MBBe; MBBI++)
			{
				...
				Phi = &RegionNext->instr_front();
				unsigned p = Copy->getOpcode();
				if (Phi&&Phi->isPHI() && Copy->isCopy()&&isConditionCode(ConditionCode)) {
...
{
	unsigned CPR = Copy->getOperand(0).getReg();
	bool flag = false;
	...
	unsigned S_reg = Copy->getOperand(1).getReg();
	const TargetRegisterClass *RC = MRI->getRegClass(Copy->getOperand(0).getReg());
	MBBI->erase(Copy);
